...
```{r, include = FALSE}
run_pipeline(targets_proj_name = "_load_per_case", submit = TRUE)
...
```{r, include = FALSE}
run_pipeline(targets_proj_name = "_load_variation", submit = TRUE)
if(length(tar_read(jobs_EpiSewer_invalid))>0) {
  warning("There are invalid jobs.")
}
```
...
inf_plot <- plot_infections(plot_res, forecast = FALSE, median = TRUE) + 
...
R_plot <- plot_R(c(plot_res, list(baseline = job_EpiSewer_result[[baseline_select$i]])), forecast = FALSE, base_model = "baseline", median = TRUE) + 
...
load_plot <- plot_load(plot_res, forecast = FALSE, median = TRUE) + 
  coord_cartesian(ylim = c(0,NA)) + 
  facet_wrap(~model, scales = "free_y", ncol = 1) + 
  theme(
    legend.position = "none",
