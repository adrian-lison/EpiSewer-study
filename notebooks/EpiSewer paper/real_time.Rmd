...
Check all results
```{r}
all_pipelines <- purrr::list_c(lapply(c(
  "_real_time_selection_targets_22-23",
  "_real_time_selection_targets_23-24",
  "_real_time_selection_targets_24-25"), function(x) {
...
write.csv(R_coverage_week, here::here("data", "results", "EpiSewer paper", paste0("R_real_time_coverage_", stringr::str_replace(season_select,"/","-"),".csv")), row.names = FALSE)
...
write.csv(R_median_error_week, here::here("data", "results", "EpiSewer paper", paste0("R_real_time_median_error_", stringr::str_replace(season_select,"/","-"),".csv")), row.names = FALSE)
...
write.csv(bias_plot_data, here::here("data", "results", "EpiSewer paper", paste0("forecast_bias_", stringr::str_replace(season_select,"/","-"),".csv")), row.names = FALSE)
...
write.csv(coverage_data, here::here("data", "results", "EpiSewer paper", paste0("forecast_coverage_", stringr::str_replace(season_select,"/","-"),".csv")), row.names = FALSE)
...
## All real-time results as tables
```{r}
library(kableExtra)
wwtps_select <- c("Zurich" = "ARA Werdhoelzli", "Lugano" = "CDA Lugano", "Chur" = "ARA Chur")
```
...
```{r}
R_consistency_week_all <- bind_rows(lapply(c("2022/23", "2023/24", "2024/25"), function(season) {
  coverage <- readr::read_csv(here::here("data", "results", "EpiSewer paper", paste0("R_real_time_coverage_", stringr::str_replace(season,"/","-"),".csv")), show_col_types = FALSE )
  median_error <- readr::read_csv(here::here("data", "results", "EpiSewer paper", paste0("R_real_time_median_error_", stringr::str_replace(season_select,"/","-"),".csv")), show_col_types = FALSE)
  full_join(coverage, median_error, by = c("wwtp", "target", "h_week")) |> 
    mutate(season = season)
})) |> 
  mutate(
    target = factor(target, levels = c("SARS-CoV-2", "IAV", "RSV"), ordered = T)
  )

R_consistency_table <- R_consistency_week_all |> 
  mutate(wwtp = factor(wwtp, levels = c(names(wwtps_select), "all"), ordered = T)) |> 
  filter(h_week >= 0, h_week <= 4) |>
  arrange(target, season, h_week, wwtp) |> 
  mutate(coverage = scales::percent(coverage, accuracy = 1), MAE = round(MAE, 2))

R_consistency_table_wide <- R_consistency_table |>
  select(Pathogen = target,
         Season = season,
         Lag = h_week,
         Location = wwtp,
         Coverage = coverage,
  ...
  tidyr::pivot_wider(
    names_from = Lag,
    values_from = c(Coverage, MAE),
    names_glue = "{.value}_{Lag}"
  ) |>
  arrange(Pathogen, Season, Location)

colnames(R_consistency_table_wide) <- c(
  "Pathogen", "Season", "Location",
  paste0("0",""), paste0("1",""), paste0("2",""), paste0("3",""), paste0("4",""),
  paste0("0",""), paste0("1",""), paste0("2",""), paste0("3",""), paste0("4","")
)

# export as latex table
R_consistency_table_latex <- R_consistency_table_wide |>
  knitr::kable(
    format = "latex",
    booktabs = TRUE,
    longtable = TRUE,
    align = paste0("lll", paste(rep("c", 10), collapse = "")),
    caption = "Real-time consistency of $R_t$ estimates across pathogens, seasons and locations."
  ) |>
  add_header_above(
    c(" " = 3,
      "Coverage (0-4 weeks lag)" = 5,
      "MAE (0-4 weeks lag)" = 5)
  ) |> 
...
```{r}
forecast_bias_all <- bind_rows(lapply(c("2022/23", "2023/24", "2024/25"), function(season) {
  readr::read_csv(
  here::here("data", "results", "EpiSewer paper", paste0("forecast_bias_", stringr::str_replace(season,"/","-"),".csv")),
  show_col_types = FALSE) |> 
  mutate(
    target = factor(target, levels = c("SARS-CoV-2", "IAV", "RSV"), ordered = T),
    wwtp = factor(wwtp, levels = names(wwtps_select), ordered = T),
    infection_trend_type = factor(infection_trend_type, levels = c("increasing", "decreasing")),
    season = season
  )
}))

forecast_bias_table_wide <- forecast_bias_all |>
  mutate(
    infection_trend_type = paste("Infections", infection_trend_type),
    bias = scales::percent(bias, accuracy = 0.1)
    ) |> 
  select(Pathogen = target,
         Season = season,
         Location = wwtp,
         infection_trend_type,
         bias) |>
  tidyr::pivot_wider(
    names_from = infection_trend_type,
    values_from = bias#, names_glue = "{.value}"
  ) |>
  arrange(Pathogen, Season, Location)

forecast_bias_table_latex <- forecast_bias_table_wide |>
  knitr::kable(
    format = "latex",
    booktabs = TRUE,
    longtable = TRUE,
    align = paste0("lll", paste(rep("c", 2), collapse = "")),
    caption = "Bias of concentration forecasts across pathogens, seasons, and locations."
  ) |>
  add_header_above(
    c(" " = 3,
      "Bias" = 2)
...
diagnostics_index <- cbind(results_index, rbindlist(lapply(job_EpiSewer_result, function(res){
  if (!"metainfo" %in% names(res$job)) {
    return(data.frame(n_chains = 4))
  }
  n_chains <- res$job$fit_opts$sampler$chains
  time_window <- with(res$job$metainfo, T_end_date - T_start_date)
  runtime <- ifelse("total" %in% names(res$runtime), res$runtime$total, NA)
  avg_runtime <- ifelse("chains" %in% names(res$runtime), mean(res$runtime$chains$total), NA)
...
diagnostics_index |> 
  filter(!is.na(target)) |> 
  mutate(time_window_weeks = 4 * (1 + (as.numeric(time_window) %/% (7*4)))) |>
  filter(time_window_weeks <= 4*10) |>
  ggplot(aes(x = as.factor(time_window_weeks), y = avg_runtime / 60, color = target)) +
  geom_boxplot() +
  ylab("Per-chain runtime [min]") +
  xlab("Length of time window [weeks]") +
  scale_x_discrete(labels = function(x) paste0(as.numeric(x)-3, "-", x)) +
  scale_color_manual(name = "Pathogen", values = palette_targets) +
  theme_bw() +
  theme(
    legend.position = "top"
...
ggsave(here::here("figures", "real_time", "diagnostics_runtime.pdf"), width = 10, height = 5)
```

```{r}
# estimate slope of runtime~time_window for each combination of target and wwtp
runtime_slopes <- diagnostics_index |>
  filter(!is.na(target)) |>
  group_by(target) |>
  mutate(time_window_weeks = as.numeric(time_window) / 7, avg_runtime_min = avg_runtime/60) |>
  summarize(
    slope = coef(lm(avg_runtime_min ~ time_window_weeks))[2],
    p_value = summary(lm(avg_runtime_min ~ time_window_weeks))$coefficients[2,4],
    .groups = "drop")

runtime_slopes

mean(runtime_slopes$slope)
```
...
# 1 month
  diagnostics_index |> 
  filter(!is.na(target)) |> 
  filter(time_window > 4 * 7 - 3, time_window < 4 * 7 + 3) |> 
  summarize(avg_runtime_mins = mean(avg_runtime, na.rm = T)/60)

# 6 months
  diagnostics_index |> 
  filter(!is.na(target)) |> 
  filter(time_window > 26 * 7 - 3, time_window < 26 * 7 + 3) |> 
  summarize(avg_runtime_mins = mean(avg_runtime, na.rm = T)/60)
```
...
diagnostics_theme_ts(
  diagnostics_index |> filter(!is.na(target)) |> 
    ...
    scale_y_continuous(labels = scales::percent_format(accuracy = 1), breaks = seq(0,0.6,0.05)) +
    ylab("Divergent transitions") + 
    coord_cartesian(ylim = c(0, 0.15))
  )
