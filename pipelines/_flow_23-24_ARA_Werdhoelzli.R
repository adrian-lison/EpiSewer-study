...
modeling_targets$sewage_target <- tar_target(
  module_sewage,
  list(
    with_flow = model_sewage(
      flows = flows_observe(),
      residence_dist = residence_dist_assume(residence_dist = c(1))
    ),
    constant_flow = model_sewage(
      # median flow for ARA Werdhoelzli
      flows = flows_assume(flow_constant = 2.06e+11),
      residence_dist = residence_dist_assume(residence_dist = c(1))
    )
  )
)
